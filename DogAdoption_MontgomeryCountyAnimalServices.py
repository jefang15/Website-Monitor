"""
Scrapes the Montgomery County Animal Services site (24petconnect.com) for dogs available for adoption and alerts me when a new
dog is available or was adopted. Prevents needing to frequently refresh the page and manually identifying changes.

https://www.montgomerycountymd.gov/animalservices/index.html

"Hidden Gems are animals who have been available for over 30 days. These animals are marked with a diamond on their photo and
are labeled as Hidden Gems in their bios. Their adoption fees are $0 thanks to Friends of Montgomery County Animals (FMCA) who
have agreed to sponsor their adoption fees!"
"""
...
def scrape_html(_website_url):
    ...
    for _card in _dog_cards:
        _picture_tag = _card.find('img', id=lambda x: x and x.startswith('AnimalImage'))
        _picture_img_url = _picture_tag['src'].strip() if _picture_tag else None
        _picture_url = 'https://24petconnect.com' + _picture_img_url

        _name_tag = _card.find('span', class_='text_Name')
        _name = _name_tag.text.strip() if _name_tag else None

        # Extract the ID from the name if it's formatted as "Name (ID)"
        _id = None
        if _name and '(' in _name and ')' in _name:
            _id = _name.split('(')[-1].strip(')')

        # Extract only the name without the ID
        if _name:
            _name = _name.split('(')[0].strip()

        _gender_tag = _card.find('span', class_='text_Gender')
        _gender = _gender_tag.text.strip() if _gender_tag else None

        _breed_tag = _card.find('span', class_='text_Breed')
        _breed = _breed_tag.text.strip() if _breed_tag else None

        _age_tag = _card.find('span', class_='text_Age')
...
def send_email(_from_email, _to_email, _email_subject, _creds, _new_dogs, _adopted_dogs, _directory_photos, _website_url,
    ...
    :param _shelter_name: Name of dog shelter.
    :return:
    """
...
def main():
    ...
    _website_url = 'https://24petconnect.com/MONT?at=DOG'

    _directory_previous_availability = 'Output - Dog Adoption - Montgomery County/Montgomery_Previous_Availability.json'
    _directory_historical_availability = 'Output - Dog Adoption - Montgomery County/Montgomery_Historical_Availability.json'
    _directory_photos = 'Output - Dog Adoption - Montgomery County/Photos'
    ...
    _shelter_name = 'Montgomery County Animal Services'
    ...
    while True:
        ...
        for page in range(_pages + 1):
            if page == 0:
                ...
                website_url_page_n = f'https://24petconnect.com/MONT?index={count}&at=DOG'
...
# website_url = 'https://24petconnect.com/MONT?at=DOG'
# directory_previous_availability = 'Output - Dog Adoption - Montgomery County/Montgomery_Previous_Availability.json'
# directory_historical_availability = 'Output - Dog Adoption - Montgomery County/Montgomery_Historical_Availability.json'
# directory_photos = 'Output - Dog Adoption - Montgomery County/Photos'
...
# shelter_name = 'Montgomery County Animal Services'
